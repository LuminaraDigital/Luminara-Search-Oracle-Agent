/**
 * 3-State Adaptive Circuit Breaker
 * Adapted from OmniRoute (src/lib/resilience/adaptiveCircuit.ts, MIT License).
 *
 * Implements Closed -> Open -> Half-Open state machine to fast-fail failing AI providers
 * and automatically probe for recovery.
 */
...
export function isCircuitAvailable(
  ...
  // If open, check if probe window has arrived
  if (circuit.state === 'open' && circuit.nextProbeAt && now >= new Date(circuit.nextProbeAt)) {
    return true;
  }
  return false;
}
...
export function observeCircuit(
  ...
  const next: AdaptiveCircuit = { ...current };
