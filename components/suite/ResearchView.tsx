    ...
    <div className="max-w-4xl mx-auto px-4 py-8 animate-in fade-in duration-700">
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#BF953F]/10 border border-[#BF953F]/30 mb-4">
          <ICONS.Research className="w-4 h-4 text-[#FCF6BA]" />
          <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#FCF6BA]">Grounded Intelligence Protocol</span>
        </div>
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
                mode === 'web' 
                  ? 'bg-gradient-to-r from-[#BF953F] to-[#AA771C] text-black shadow-lg shadow-[#BF953F]/20' 
      ...
      {result && (
        <div className="space-y-6 animate-in fade-in duration-500">
          <div className="glass-morphism rounded-2xl border border-[#BF953F]/40 p-6 sm:p-8 shadow-2xl">
            <div className="flex items-center gap-3 border-b border-white/10 pb-4 mb-6">
              <div className="w-2.5 h-2.5 rounded-full bg-[#BF953F]"></div>
              <h3 className="text-base font-bold uppercase tracking-widest text-[#FCF6BA]">Grounded Intelligence Brief</h3>
            </div>
            <div 
          ...
          {result.chunks && result.chunks.length > 0 && (
            <div className="glass-morphism rounded-2xl border border-white/10 p-6">
              <h4 className="text-xs font-bold uppercase tracking-widest text-[#FCF6BA] mb-4">Grounded Citations & Locations</h4>
                ...
                {result.chunks.map((c, i) => {
                  ...
                  return (
                    <a
                      ...
                      className="p-3 rounded-xl bg-black/60 border border-white/5 hover:border-[#BF953F]/40 flex items-center justify-between text-xs text-gray-300 hover:text-white transition-all group"
                    >
                      <span className="truncate pr-2 font-mono">{item.title || item.uri}</span>
                      <ICONS.ExternalLink className="w-3.5 h-3.5 text-[#BF953F] shrink-0" />
                    </a>
                  );
                })}
              </div>
