...
export const CliTerminalPanel: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {quickCommands.map(cmd => (
          <button
            ...
            className="px-2.5 py-1 rounded-md bg-white/[0.03] hover:bg-gold/20 border border-white/5 hover:border-gold/40 text-[10px] font-mono text-gray-300 hover:text-gold-light transition-all"
          >
            {cmd}
          </button>
        ))}
      ...
      {/* Terminal Window */}
      <div className="glass-morphism rounded-2xl border border-gold/40 bg-black/90 shadow-2xl overflow-hidden flex flex-col h-[520px]">
        {/* Terminal Header Bar */}
        <div className="px-4 py-2.5 bg-black/80 border-b border-white/10 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-danger-500/80 inline-block" />
        ...
        <div className="p-3 bg-black/95 border-t border-gold/20 flex items-center gap-2 shrink-0">
          <span className="text-success-400 font-mono text-sm font-bold pl-2">➜</span>
          ...
          <button
            ...
            className="px-3 py-1 rounded bg-gold/20 hover:bg-gold/30 border border-gold/50 text-gold-light text-[10px] font-mono uppercase tracking-wider transition-all disabled:opacity-30"
          >
            Run
          </button>
        </div>
