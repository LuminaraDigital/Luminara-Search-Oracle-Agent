...
export const SwitchyardPanel: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {/* Top Banner */}
      <div className="p-6 rounded-2xl glass-morphism border border-[#BF953F]/30 bg-gradient-to-r from-black via-white/[0.02] to-black relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
          <span className="text-9xl font-mono text-[#FCF6BA]">⮀</span>
        </div>
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="px-2.5 py-0.5 rounded-full text-[9px] font-mono uppercase tracking-wider font-bold bg-[#BF953F]/20 text-[#FCF6BA] border border-[#BF953F]/40">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => switchyardRouterService.clearLogs()}
              className="px-3 py-1.5 rounded-xl border border-white/10 hover:border-red-500/40 text-[10px] font-mono text-gray-400 hover:text-red-400 transition-all"
            >
              Reset Metrics
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-white/5">
          ...
          <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
            ...
            <div className="text-2xl font-black text-emerald-400 mt-1 font-mono">
              ~58.4%
              <span className="text-[10px] font-normal text-emerald-500/70 ml-2">
          ...
          <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
            ...
            <div className="text-2xl font-black text-[#FCF6BA] mt-1 font-mono">
              3.8x
      ...
      <div className="p-6 rounded-2xl glass-morphism border border-white/10 bg-black/60 space-y-4">
        <div className="flex items-center justify-between border-b border-white/5 pb-3">
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider text-[#FCF6BA]">Routing Topology Architecture</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 pt-2 text-center text-xs">
          ...
          {/* Node 2: Algorithm */}
          <div className="p-4 rounded-xl border border-[#BF953F]/40 bg-[#BF953F]/10 flex flex-col items-center justify-center">
            <span className="text-xl mb-1">⚡</span>
            <span className="font-bold text-[#FCF6BA]">Switchyard Core</span>
            <span className="text-[9px] font-mono text-[#BF953F] mt-1">
          ...
          {/* Node 3: Targets */}
          <div className="p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 flex flex-col items-center justify-center">
            <span className="text-xl mb-1">🎯</span>
            <span className="font-bold text-emerald-400">Target Dispatched</span>
            <span className="text-[9px] font-mono text-emerald-300 mt-1">Groq LPU / NIM / Gemini</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl glass-morphism border border-white/10 bg-black/60 space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/5 pb-4">
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider text-[#FCF6BA]">Interactive Workload Route Simulator</h3>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-black/80 border border-white/15 focus:border-[#BF953F] rounded-xl px-3 py-1.5 text-xs text-[#FCF6BA] font-mono focus:outline-none"
            >
              {routes.map(r => (
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {samplePrompts.map((p, idx) => (
              <button
                ...
                className="px-2.5 py-1 rounded-full text-[10px] font-mono bg-white/5 hover:bg-[#BF953F]/20 text-gray-300 hover:text-[#FCF6BA] border border-white/10 hover:border-[#BF953F]/40 transition-all"
              >
                {p.label}
              </button>
            ))}
          ...
          <textarea
            ...
            className="w-full bg-black/80 border border-white/15 focus:border-[#BF953F] rounded-xl p-3 text-xs text-white font-mono placeholder:text-gray-600 focus:outline-none transition-all leading-relaxed"
          />
          ...
          <div className="flex items-center justify-end gap-3 pt-1">
            ...
            <button
              ...
              disabled={isSimulating}
              className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#BF953F] to-[#AA771C] text-black text-xs font-black uppercase tracking-wider shadow-lg hover:scale-105 active:scale-95 transition-all flex items-center gap-2"
            >
              {isSimulating ? (
                <>
        ...
        {activeSimulationResult && (
          <div className="p-4 rounded-xl bg-white/[0.02] border border-[#BF953F]/30 space-y-3 animate-in fade-in">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-mono uppercase tracking-wider text-[#FCF6BA] font-bold">
            ...
            <div className="space-y-2 pt-1">
              {activeSimulationResult.log.steps.map((step, sIdx) => (
                <div key={sIdx} className="flex items-start gap-3 text-xs font-mono p-2.5 rounded-lg bg-black/40 border border-white/5">
                  <span className="px-2 py-0.5 rounded bg-white/10 text-[#FCF6BA] text-[10px] shrink-0">
                    Step {sIdx + 1}
                  </span>
                  <div>
                    <span className="text-gray-300 font-bold">{step.stage}</span>
                    <span className="text-gray-500 mx-2">➜</span>
                    <span className="text-emerald-400">{step.chosenTarget}</span>
      ...
      <div className="p-6 rounded-2xl glass-morphism border border-white/10 bg-black/60 space-y-4">
        <div className="flex items-center justify-between border-b border-white/5 pb-3">
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider text-[#FCF6BA]">Switchyard Target Registry</h3>
                ...
                <tr key={t.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="py-3 px-3 font-bold text-gray-200 flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${
                      t.role === 'efficient' ? 'bg-emerald-400 shadow-[0_0_6px_#10B981]' : t.role === 'capable' ? 'bg-[#BF953F] shadow-[0_0_6px_#BF953F]' : 'bg-purple-400'
                    }`}></span>
                    <span>{t.name}</span>
                  </td>
                  <td className="py-3 px-3">
                    <span className={`px-2 py-0.5 rounded text-[9px] uppercase font-bold ${
                      t.role === 'efficient' ? 'bg-emerald-500/10 text-emerald-400' : t.role === 'capable' ? 'bg-[#BF953F]/10 text-[#FCF6BA]' : 'bg-purple-500/10 text-purple-400'
                    }`}>
                      {t.role}
                    </span>
                  </td>
                  <td className="py-3 px-3 uppercase text-gray-400">{t.provider}</td>
                  <td className="py-3 px-3 text-[#FCF6BA]">{t.tokensPerSec} tok/s</td>
